...
- Treat final shutdown and temporary handoff as different boundaries. For an editor, shell, or supported suspend, prefer the framework's handoff API: pause UI input, restore the shell-facing terminal, wait, re-enter modes, refresh stale state, and force a full redraw. Do not final-unmount an app that must resume, and do not assume POSIX signals exist on Windows.
